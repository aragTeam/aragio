(function(f, g) {
    function Ra() {
        ...
        ya();
        setInterval(ya, 18E4);
        A = la = document.getElementById("canvas");
        e = A.getContext("2d");
        ...
        f.onresize = Ca;
        Ca();
        f.requestAnimationFrame ? f.requestAnimationFrame(Da) : setInterval(na, 1E3 / 60);
        setInterval(K, 40);
        u && g("#region").val(u);
        Ea();
        V(g("#region").val());
    ...
    function ya() {
        ...
        function(a) {
            ...
            for (c in b) g('#region option[value="' + c + '"]').text(Z[c] + " (" + b[c] + " players)")
        },
        "json")
    }
    function Fa() {
    ...
    function V(a) {
        a && a != u && (g("#region").val() != a && g("#region").val(a), u = f.localStorage.location = a, g(".region-message").hide(), g(".region-message." + a).show(), g(".btn-needs-server").prop("disabled", !1), ka && W())
    }
    function Ba(a) {
        D = null;
    ...
    function na() {
        ...
        Sa();
        ma();
        ua || e.clearRect(0, 0, n, r);
        if (ua) e.fillStyle = ga ? "#111111": "#F2FBFF",
        e.globalAlpha = .05,
        e.fillRect(0, 0, n, r),
        e.globalAlpha = 1;
        else {
            e.fillStyle = ga ? "#111111": "#F2FBFF";
            e.fillRect(0, 0, n, r);
            e.save();
            e.strokeStyle = ga ? "#AAAAAA": "#000000";
            e.globalAlpha = .2;
            e.scale(l, l);
            b = n / l;
            a = r / l;
            ...
            e.lineTo(d, a),
            e.stroke();
            for (d = -.5 + ( - t + a / 2) % 50; d < a; d += 50) e.beginPath(),
            e.moveTo(0, d),
            e.lineTo(b, d),
            e.stroke();
            e.restore()
        ...
        if (ra) {
            ...
            e.globalAlpha = .5;
            e.beginPath();
            for (d = 0; d < h.length; d++) e.moveTo(h[d].x, h[d].y),
            e.lineTo(aa, ba);
            e.stroke();
            e.restore()
        }
        ...
        I = Math.max(I, ab());
        0 != I && (null == ha && (ha = new ia(24, "#FFFFFF")), ha.setValue("Score: " + ~~ (I / 100)), a = ha.render(), b = a.width, e.globalAlpha = .2, e.fillStyle = "#000000", e.fillRect(10, r - 10 - 24 - 10, b + 10, 34), e.globalAlpha = 1, e.drawImage(a, 15, r - 10 - 24 - 5));
        bb();
        c = +new Date - c;
    ...
    Ta = "https:" == G;
    if ("agar.io" != f.location.hostname && "localhost" != f.location.hostname && "10.10.2.13" != f.location.hostname) f.location = G + "//agar.io/";
    else if (f.top != f) f.top.location = G + "//agar.io/";
    else {
        var la, e, A, n, r, L = null,
        m = null,
        s = 0,
        t = 0,
        E = [],
        ...
        if ("undefined" == typeof console || "undefined" == typeof DataView || "undefined" == typeof WebSocket || null == Qa || null == Qa.getContext || null == f.localStorage) alert("You browser does not support this game, we recommend you to use Firefox to play this");
        else {
            var Z = null;
            ...
            null != f.localStorage && (null == f.localStorage.AB8 && (f.localStorage.AB8 = 0 + ~~ (100 * Math.random())), Pa = +f.localStorage.AB8, f.ABGroup = Pa);
            g.get(G + "//gc.agar.io",
            function(a) {
                var b = a.split(" ");
                a = b[0];
            ...
            Ja.prototype = {
                ...
                draw: function() {
                    if (this.shouldRender()) {
                        ...
                        wa ? (e.fillStyle = "#FFFFFF", e.strokeStyle = "#AAAAAA") : (e.fillStyle = this.color, e.strokeStyle = this.color);
                        if (a) e.beginPath(),
                        ...
                        a || e.stroke();
                        e.fill();
                        null == d || b || (e.save(), e.clip(), e.drawImage(d, this.x - this.size, this.y - this.size, 2 * this.size, 2 * this.size), e.restore()); (wa || 15 < this.size) && !a && (e.strokeStyle = "#000000", e.globalAlpha *= .1, e.stroke());
                        e.globalAlpha = 1;
